
using System;
using System.Collections.Generic;
using System.Linq;
using System.Text;
using FluentNHibernate.Mapping;

namespace Model.Mapping
{
    public class ScenicMap : ClassMap<Scenic>
    {
        public ScenicMap()
        {
            Id(x => x.Id);
            Map(x => x.Name);
            Map(x => x.Address);
            Map(x => x.ScenicOrder);
            Map(x => x.Level);
            Map(x => x.Photo);
            //Map(x => x.ActiveTime);
            Map(x => x.Trafficintro).CustomSqlType("ntext");
            Map(x => x.ScenicDetail).CustomSqlType("ntext");
            Map(x => x.Desec).CustomSqlType("ntext");
            Map(x => x.Position);
            Map(x => x.SeoName);
            Map(x => x.BookNote).CustomSqlType("ntext");
            Map(x => x.TransGuid).CustomSqlType("ntext");
            References<Area>(x => x.Area);
            HasMany<ScenicCheckProgress>(x => x.CheckProgress);
            HasMany<Ticket>(x => x.Tickets);
        }
    }
    public class ScenicCheckProgressMap : ClassMap<ScenicCheckProgress>
    {
        public ScenicCheckProgressMap()
        {
            Id(x => x.Id);
            References<TourMembership>(x => x.Applier);
            References<TourMembership>(x => x.Checker);
            Map(x => x.CheckMessage);
            Map(x => x.CheckStatus).CustomType<int>();
            Map(x => x.CheckTime);
            Map(x => x.Module).CustomType<int>();
            References<Scenic>(x => x.Scenic);
        }
    }
    public class ScenicImgMap : ClassMap<ScenicImg>
    {
        public ScenicImgMap() {
            Id(x => x.Id);
            References<Scenic>(x => x.Scenic);
            Map(x => x.Title);
            Map(x => x.Description);
            Map(x => x.ImgType).CustomType<int>();
            Map(x => x.Name);
        }
    }
}
